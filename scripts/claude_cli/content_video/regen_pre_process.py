...
from scripts.claude_cli.content_video.pre_process import VideoScenePreProcess
from scripts.controllers.utils.decorators.try_catch import try_catch


class VideoSceneRegenPreProcess(VideoScenePreProcess):
    ...
    def save_prompt(self):
        ...
        for scene_index in self.scenes:
            ...
            file_path = self.prompt_path.format(scene_index=scene_index)
            self.logger.info(f"Saved video scene prompt to: {file_path}")
...
if __name__ == "__main__":
    ...
    pre_process = VideoSceneRegenPreProcess(topic=args.topic, scenes=scene_indices)
    pre_process.run()
